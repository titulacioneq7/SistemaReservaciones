...
// Reservation form submission for Terapias Estéticas
const formEsteticas = document.getElementById('appointmentFormEsteticas');
if (formEsteticas) {
  formEsteticas.addEventListener('submit', (e) => {
    e.preventDefault();
...
const calendarElEsteticas = document.getElementById('calendarEsteticas');
if (calendarElEsteticas) {
  document.addEventListener('DOMContentLoaded', () => {
    const calendar = new FullCalendar.Calendar(calendarElEsteticas, {
      initialView: 'dayGridMonth',
      events: function(fetchInfo, successCallback, failureCallback) {
        db.collection('appointments_esteticas').get().then(querySnapshot => {
          let events = [];
      ...
      eventClick: function(info) {
        if (confirm('¿Deseas eliminar esta cita?')) {
          db.collection('appointments_esteticas').doc(info.event.id).delete()
            .then(() => {
      ...
      eventDrop: function(info) {
        ...
        db.collection('appointments_esteticas').doc(info.event.id).update({
          date: newDate,
          time: newTime
        })
        .then(() => {
